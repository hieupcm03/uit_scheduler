  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        let allCombinedData: T[] = [];

        // SheetNames là mảng chứa tên các Sheet
        for (const sheetName of workbook.SheetNames) {
          const worksheet = workbook.Sheets[sheetName]; //Sheets là object dạng <key, value> chứa dữ liệu của các Sheet
          ...
          if (headerRowIndex === -1) {
            continue; // Bỏ qua sheet rỗng hoặc sheet không chứa cấu trúc môn học
          }
          ...
          allCombinedData = [...allCombinedData, ...jsonData];
        }
